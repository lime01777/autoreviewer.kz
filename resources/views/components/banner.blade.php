...
@if($banner)
    <div class="banner-container my-8 overflow-hidden rounded-2xl shadow-sm border border-gray-100 bg-white group transition duration-500 hover:shadow-xl">
        <a href="{{ $banner->link ?? '#' }}" {{ $banner->link ? 'target="_blank" rel="noopener noreferrer"' : '' }} class="block relative">
            <img src="{{ $banner->image_url }}" alt="{{ $banner->title }}" class="w-full h-auto object-cover transition duration-700 group-hover:scale-[1.02]" loading="lazy">
